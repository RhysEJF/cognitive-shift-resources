...
Anyone running a markdown-based AI Second Brain who wants to upgrade the search layer without rebuilding their setup. Works for:

- Flow OS variants and forks
- Tiago Forte / PARA structures
- Cole Medin / Bryan Brown templates  
- Hand-rolled second brains
- Custom GPT setups with markdown vaults
- Obsidian or similar (export to markdown first)

You need:

- A folder of markdown files (your second brain)
- An AI agent that can read instructions and run shell commands (Claude Code is the smoothest path; Claude Desktop and custom GPTs work with one extra step)
...
**Risk:** low if you follow the stages in order. Stages 1-7 are reversible. Stage 8 (branding) is reversible by editing two files. Stage 9 (cleanup) removes safety nets, so do that last and only after confirmed-stable use.
...
| 7c | [Wire custom GPT or other agent](prompts/stage-7c-wire-custom-gpt.md) | Provides a small CLI wrapper for any agent that can shell out | Yes (delete the wrapper) |
| 8 | [Brand the upgrade](prompts/stage-8-brand-the-upgrade.md) | Adds attribution footer to README + Memory Layer section to CLAUDE.md | Yes (edit two files) |
...
- Your README and CLAUDE.md (Stage 8) — small additive sections, removable any time
